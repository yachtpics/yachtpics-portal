...
- **Marquee layout rework after Charlie saw renders (both looks).** Bands on 1080×1920 (`mq` in ReelMaker): **top** 14%→33.75% (269→648, 379px), **middle/title** 33.75%→52.5% (648→1008, 360px), **bottom** 52.5%→90% (1008→1728, 720px = `W / 1.5`, so a 3:2 landscape shows whole at full width). The coordinator's suggested 36.9% top edge left a 300px middle band, and the ordinary title block (lead-in, one-line name, spec, location) draws about 300px, so it would have shrunk; I took 60px from the top band so that block fits at full size with ~30px either side. The **bottom band follows the Framing chips again** (they're back for Marquee looks): *whole* (default) = the photo contained on the usual blurred backdrop, cropped to the band; *fill* = cover-crop. The top band is always cover-cropped. **Title centring:** drawTitle now places every line from one set of baseline offsets used by both the draw and the measure (identical positions on every look). On a Marquee the block is centred between the two photos on its *drawn* ink height (`measureText` ascent of the first line + offsets + descent of the last), so the gaps (and the hairlines on the photo edges) are equal above and below. Shrink-to-fit about that centre is kept, with a 20px minimum gap: a two-line spec row sets at ~95%, a two-line name at ~87%.
