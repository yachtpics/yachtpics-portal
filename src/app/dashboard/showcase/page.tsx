...
export default async function ShowcasePage() {
  ...
  // The reader is SECURITY DEFINER and only returns admin-curated boats.
  const { data } = await supabase.rpc("showcase_listings");
  const rows = (data ?? []) as ShowcaseRow[];

  // Sign the hero photographs in one batched pass (service client, since these
  // are other brokers' photos — the showcase is a curated cross-broker feed).
            ...
            return (
              <div key={r.listing_id}>
                ...
                <div className="mt-4">
                  ...
                  {/* Listing broker contact — the warm handoff */}
